...
When a developer shares a PR link and asks to "work through the comments", "review the PR comments", "phase 1", or similar, follow this exact two-phase process. It keeps analysis and implementation cleanly separated so the developer can steer each fix before any code changes.

## Phase 1 — Analysis and recommendations
- Always do analysis before any code changes. Do not modify files in Phase 1.
- **Always post findings to GitHub** when the user asks for Phase 1 (or a PR review using this workflow). Do not leave analysis only in chat — the PR is the source of truth for what to fix in Phase 2.
- Fetch PR metadata and all review comments via the `gh` CLI:
  - `gh pr view <N> --repo <owner>/<repo> --json number,title,headRefName,baseRefName,body,author,headRefOid`
...
- For each remaining unresolved comment **and each proactive finding from your own review**, produce a focused analysis. **Every analysis must contain all three labeled fields in this order — do not skip or merge them:**
...
- **Posting Phase 1 analysis to GitHub** (required):
  - **Existing review thread**: reply on that thread (see reply command below).
  - **New finding (no thread yet)**: create a line-anchored review comment on the PR head commit (`headRefOid` from `gh pr view`), on the most relevant file and line:
```bash
cat <<'EOF' > /tmp/pr-comment.json
{"body":"**Analysis**\n\n<issue>\n\n**Origin**: Introduced by this PR\n\n**Recommendation**: Fix — <rationale>","commit_id":"<headRefOid>","path":"src/example.ts","line":42,"side":"RIGHT"}
EOF
gh api --method POST repos/<owner>/<repo>/pulls/<N>/comments --input /tmp/pr-comment.json --jq '.html_url'
```
  - **PR-wide finding** (CI, migration scope, cross-file): post an issue comment on the PR:
```bash
...
EOF
gh api --method POST repos/<owner>/<repo>/pulls/comments/<comment_id>/replies --input /tmp/reply.json --jq '.html_url'
```
- After posting all analyses to GitHub, present the same summary table in chat (columns: Thread / location, Reviewer, Origin, Recommendation). Do not start applying fixes until the developer explicitly says what to fix.
...
- **After each fix, always post a reply on the corresponding GitHub thread** explaining what you did. Use this structure:
  - **Fix applied** (or **Skipped** if the developer directed you not to fix)
  - What changed (files, behavior)
  - Tests added/updated and command run + result
  - For duplicate threads: shorter "same fix covers this thread too" reply on secondary thread(s).
  - Use the same JSON payload pattern as Phase 1 (`/tmp/reply.json` + `gh api .../replies` for thread replies, or a new comment on the same path/line if no thread id exists).
- Never commit or push — leave that to the developer.
- End with a concise summary in chat: files changed, new/updated tests, verification evidence, GitHub reply links, and anything skipped per the developer's direction.
