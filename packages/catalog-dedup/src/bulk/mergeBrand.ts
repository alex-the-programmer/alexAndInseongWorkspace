...
export function resolveBrandMergePlan(members: BrandWithProductCount[]): {
  canonicalBrandId: bigint;
  duplicateBrandIds: bigint[];
  displayName: string;
} {
...
export default async function executeBrandMerge(
  prisma: PrismaClient,
  params: ExecuteBrandMergeParams
): Promise<ExecuteBrandMergeResult> {
  ...
  const resolvedDisplayName =
    displayName ?? pickDisplayBrandName(membersWithCounts, canonicalMember);
