...
function productsBySeller(products: DedupProduct[]): Map<string, DedupProduct[]> {
  const bySeller = new Map<string, DedupProduct[]>();
...
function sellerIdsInComponent(
  productIds: string[],
  productById: Map<string, DedupProduct>
): Set<string> {
...
export function estimatePairwiseEdgesForBrand(params: {
  ...
  for (let i = 0; i < sellerIds.length; i++) {
    for (let j = i + 1; j < sellerIds.length; j++) {
      ...
      if (productsA.length === 0 || productsB.length === 0) continue;
...
export function estimatePairwiseEdges(params: {
  brandBuckets: Array<{ brandId: bigint; brandName: string; products: DedupProduct[] }>;
  sellerNamesById: Map<string, string>;
  ...
  const {
    ...
    maxOyAbsentSamples = 20,
  } = params;

  const sellerPairCounts = new Map<string, SellerPairEdgeCount>();
  const oyAbsentClusterSamples: OyAbsentClusterSample[] = [];
  ...
  for (const bucket of brandBuckets) {
    ...
    for (const productIds of components) {
      ...
      if (oyAbsentClusterSamples.length >= maxOyAbsentSamples) continue;
  ...
  const oyAbsentThreePlusSellerComponentPct =
    threePlusSellerComponents === 0
      ? 0
