...
export function collapseWhitespace(name) {
...
export function isPromoBracketTag(tag) {
    const t = normalizeTagText(tag);
    if (!t)
        return false;
    if (/\bbogo\b/i.test(tag))
        return true;
    if (t === "new" || t === "sale" || t === "gift" || t === "hot" || t === "best")
        return true;
    if (t === "bundle" || t === "limited" || t === "set")
        return true;
    if (t.includes("time deal"))
        return true;
    if (t === "oy exclusive" || t === "tk only")
        return true;
    if (t === "stylekorean")
        return true;
    if (/^\d+\s*\+\s*\d+/.test(t))
        return true;
    if (/^\d+\s*\+\s*\d+\s*set$/i.test(t))
        return true;
    return false;
}
const LEADING_BRACKET_SEGMENT = /^\s*\[([^\]]+)\]\s*/;
function stripLeadingBarePromoWord(name) {
    const match = name.match(/^(bogo|new|sale|gift|hot|best|limited)\s+/i);
    if (!match)
        return name;
    const rest = name.slice(match[0].length);
    if (!rest || !/[A-Za-z0-9\uac00-\ud7af]/.test(rest[0]))
        return name;
    return rest;
}
/** Remove leading promo bracket tags and asterisk markers allowed by `isPromoBracketTag`. */
export function stripPromotionalPrefixes(name) {
    let result = name;
    for (;;) {
        const bracket = result.match(LEADING_BRACKET_SEGMENT);
        if (!bracket || !isPromoBracketTag(bracket[1]))
            break;
    ...
    for (;;) {
        ...
        if (!["new", "sale", "hot", "best", "gift", "limited"].includes(inner))
            break;
